...
def parse_config():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1, help="batch size during inference"
    )
    parser.add_argument("--base_model", type=str, default="checkpoints/llama2")
    parser.add_argument("--cache_dir", type=str, default="./cache")
    parser.add_argument(
        "--seq_len", type=int, default=16384, help="context length during evaluation"
    )
    parser.add_argument(
        "--context_size", type=int, default=-1, help="context size during fine-tuning"
    )
    parser.add_argument(
        "--peft_model", type=str, default="checkpoints/llama_peft_red", help=""
    )
    parser.add_argument(
        "--data_path", type=str, default="dataset/test_pg19.bin", help=""
    )
...
def get_as_batch(data, seq_length, batch_size, device="cpu", sliding_window=256):
    ...
    for idx in range(0, len(all_ix), batch_size):
        ...
        x = torch.stack(
            [torch.from_numpy((data[i : i + seq_length]).astype(np.int64)) for i in ix]
        )
        y = torch.stack(
            [
                torch.from_numpy((data[i + 1 : i + 1 + seq_length]).astype(np.int64))
                for i in ix
            ]
        )
        if device != "cpu":
...
def evaluate(
    ...
    with torch.no_grad():
        ...
        for idx, (x, y) in tqdm(
            ...
            total=iceildiv(iceildiv(len(subset), sliding_window), batch_size),
        ):
            val_loss = 0.0
            acc = 0.0
            cnt = 0
            ...
            for part_idx, i in enumerate(range(0, x.shape[1], seq_length)):
                ...
                acc = (
                    (outputs.logits.argmax(-1) == y[:, i : i + seq_length])
                    .float()
                    .sum()
                ) + acc
                cnt += part_len
    ...
    stats["val_perplexity_per_chunk"] = torch.exp(
        torch.as_tensor(loss_step_list_val).mean(dim=1)
    )
...
def main(args):
    ...
    stats = evaluate(
        model, data, args.batch_size, device, args.seq_len, sliding_window=10240
    )
