...
DOC_LINE = re.compile(
    r"^\[(?P<mode>baseline_2d|merged_2d)\] doc (?P<idx>\d+)/(?P<total>\d+) loss=(?P<loss>[0-9.]+) dt=(?P<dt>[0-9.]+)s"
)
...
def main():
    ...
    parser.add_argument(
        "--out", default="logs/extensions/token_merging_2d/comparison_500_perdoc.csv"
    )
