...
class LMPackDataset(torch.utils.data.Dataset):
    def __init__(self, filepath):
        self.input_ids, self.attention_masks, self.labels, self.weights, self.nums = (
    ...
    def process_data(self, filepath):
        ...
        attention_masks = json.load(
            open(os.path.join(filepath, "attention_masks_pack.json"))
        )
        num_gpus = torch.cuda.device_count()
        l = (input_ids.size(0) // num_gpus) * num_gpus
        input_ids, labels, weights, attention_masks = (
            input_ids[:l, :],
            labels[:l, :],
            weights[:l, :],
            attention_masks[:l],
        ...
        nums = [
            weights[i * num_gpus : (i + 1) * num_gpus, :].sum()
            for i in range(l // num_gpus)
        ]
    ...
    def __getitem__(self, idx):
        if idx < 32:  # reduce GPU memory usage during first few steps
            ...
            return {
                ...
                "labels": (
                    self.labels[idx, :max_length_tmp],
                    self.weights[idx, :max_length_tmp] * 2,
                    self.nums[idx // self.num_gpus],
                ),
            }
        else:
            return {
                ...
                "attention_mask": torch.tensor(
                    self.attention_masks[idx], dtype=torch.int32
                ),
                "labels": (
                    self.labels[idx],
                    self.weights[idx],
