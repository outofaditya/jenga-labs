...
class Trainer:
        ...
        if args.optim == OptimizerNames.ADAFACTOR:
            ...
            if is_layerwise and args.parallel_mode == ParallelMode.DISTRIBUTED:
                raise NotImplementedError(
                    "Layer-wise GaLore does not support DDP at this time"
                )
            ...
            if model is None:
                raise ValueError(
                    "You need to pass a model in order to correctly initialize a GaLore optimizer."
                )
            ...
            non_galore_params = [
                p for n, p in model.named_parameters() if n not in galore_params_names
            ]
            ...
            if is_layerwise:
                ...
                if args.gradient_accumulation_steps != 1:
                    raise ValueError(
                        "Layerwise GaLoRE optimizer do not support gradient accumulation !"
                    )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    print(
                        f"before step allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
                    )
                    torch.cuda.reset_peak_memory_stats()
                    self.optimizer.step()
                    print(
                        f"after step allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
                    )
                    if step == 3:
                        exit(0)
                    torch.cuda.reset_peak_memory_stats()
    ...
    def training_step(
        ...
        if self.use_apex:
            ...
            print(
                f"before backward allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
            )
            torch.cuda.reset_peak_memory_stats()
            self.accelerator.backward(loss, **kwargs)
            print(
                f"after backward allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
            )
            torch.cuda.reset_peak_memory_stats()
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        print(
            f"before forward allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
        )
        torch.cuda.reset_peak_memory_stats()
        ...
        print(
            f"after forward allocaiton: {torch.cuda.memory_allocated(0) / 1024 / 1024}, peak memory: {torch.cuda.max_memory_allocated(0) / 1024 / 1024}, reserve: {torch.cuda.memory_reserved(0) / 1024 / 1024}"
        )
        torch.cuda.reset_peak_memory_stats()
            ...
            if self.args.batch_eval_metrics:
                if (
                    ...
                    if args.include_inputs_for_metrics:
                        ...
                        metrics = self.compute_metrics(
                            EvalPrediction(
                                predictions=preds_host, label_ids=labels_host
                            ),
                            compute_result=is_last_step,
                        )
