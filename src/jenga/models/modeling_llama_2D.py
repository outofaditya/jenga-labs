...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        query_states = shift(query_states, bsz, ql, group_size, self.num_heads, self.head_dim)
        key_states = shift(key_states, bsz, ql, group_size, key_states.shape[2], self.head_dim)
        value_states = shift(value_states, bsz, ql, group_size, key_states.shape[2], self.head_dim)    
        ...
        #NOTE: shift back
        attn_output = attn_output.reshape(bsz, ql, self.num_heads, self.head_dim).contiguous()
