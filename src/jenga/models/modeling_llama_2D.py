    ...
    def forward(
        ...
        if self.layer_idx != 31:
            with torch.no_grad():
                ...
                if merge_eliminated:
                    keep_mask = torch.zeros(
                        ...
                        num_blocks_total,
                        dtype=torch.bool,
                        device=kept_block_idx.device,
                    )
                    keep_mask.scatter_(1, kept_block_idx, True)
                    dropped_block_idx = torch.nonzero(~keep_mask[0], as_tuple=True)[0]

                base = torch.arange(64, device=kept_block_idx.device).view(
                    1, 1, 64
                )  # (1, 1, 64)
                expanded_idx = (
                    kept_block_idx[..., None] * 64 + base
                )  # (bsz, q_len_now, 64)
                idx = expanded_idx.view(-1)  # (bsz, q_len_now * 64)

                if merge_eliminated:
                    drop_token_idx = (
                        dropped_block_idx[:, None] * 64
                        + torch.arange(64, device=kept_block_idx.device).view(1, 64)
                    ).view(-1)
            ...
            if merge_eliminated:
                ...
                merged_vec = dropped_hidden.mean(
                    dim=1, keepdim=True
                )  # (bsz, 1, hidden)
        ...
        query_states = query_states.view(
            bsz, q_len_now, self.num_heads, self.head_dim
        ).transpose(1, 2)
        key_states = key_states.view(
            bsz, q_len_now, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)
        value_states = value_states.view(
            bsz, q_len_now, self.num_key_value_heads, self.head_dim
        ).transpose(1, 2)
        ...
        def shift(qkv, bsz, q_len, group_size, num_heads, head_dim):
            qkv = qkv.transpose(1, 2)  # -> [bsz, num_heads, q_len, head_dim]
            qkv = torch.cat(
                [
                    qkv[:, : num_heads // 2],
                    qkv[:, num_heads // 2 :].roll(-group_size // 2, dims=2),
                ],
                dim=1,
            )
            qkv = qkv.transpose(1, 2)
            qkv = qkv.reshape(
                bsz * (q_len // group_size), group_size, num_heads, head_dim
            )
            return qkv
        ...
        key_states = shift(
            key_states, bsz, ql, group_size, key_states.shape[2], self.head_dim
        )
        value_states = shift(
            value_states, bsz, ql, group_size, key_states.shape[2], self.head_dim
        )
        ...
        attn_output = attn_output.reshape(
            bsz, ql, self.num_heads, self.head_dim
        ).contiguous()
        attn_output = torch.cat(
            [
                attn_output[:, :, : self.num_heads // 2],
                attn_output[:, :, self.num_heads // 2 :].roll(group_size // 2, dims=1),
            ],
            dim=2,
        )
        # NOTE: shift back end

        attn_output = attn_output.reshape(bsz, ql, -1).contiguous()
        attn_output = self.o_proj(attn_output)
        ...
        if self.layer_idx != 31:
            ...
            output[0].scatter_(
                0, idx.unsqueeze(1).expand(-1, attn_output.size(-1)), attn_output[0]
            )
            if merge_eliminated:
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.self_attn = LLAMA_ATTENTION_CLASSES[config.attn_implementation](
            ...
            if labels is not None:
                ...
                for start in range(0, N - 1, 2048):
                    ...
                    loss_chunk = torch.utils.checkpoint.checkpoint(
                        forward_fn_chunk, x_chunk, self.lm_head, label_chunk
                    )
                    loss += loss_chunk
                loss /= N
