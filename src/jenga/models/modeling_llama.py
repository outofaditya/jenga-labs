    ...
    def forward(
        ...
        if self.layer_idx != 31:
            with torch.no_grad():
                ...
                base = torch.arange(64, device=idx_blocks.device).view(
                    1, 1, 64
                )  # (1, 1, 64)
                expanded_idx = (
                ...
                if merge_eliminated:
                    drop_token_idx = (
                        ...
                        + torch.arange(64, device=idx_blocks.device).view(1, 64)
                    ).view(-1)
                    ...
                    merge_idx = torch.tensor(
                        [merged_pos], device=idx.device, dtype=idx.dtype
                    )
        ...
        if self.layer_idx != 31:
            ...
            if merge_eliminated:
                ...
                output[0].scatter_(
                    0,
                    kept_idx.unsqueeze(1).expand(-1, attn_output.size(-1)),
                    attn_output[0, :n_kept],
                )
