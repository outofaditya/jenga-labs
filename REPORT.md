...
Long-context LoRA fine-tuning is bounded by activation memory, not weight memory. Jenga (Wang et al., USENIX ATC 2025) attacks this ceiling by training a small per-block attention predictor that decides — per layer, per batch — which token blocks to discard from the heavy matmuls, exchanging a small perplexity penalty for a large memory and step-time saving. We reproduce the paper's principal memory, time, memory and time breakdown, algorithm ablation, predictor convergence, scalability, and perplexity results on a single rented GPU at sequence lengths up to 16 K, matching the authors' shipped numbers within approximately one percent on every axis. We then introduce **Token Merging for Jenga**, a one-flag runtime extension that replaces Jenga's hard token elimination with a soft variant in the spirit of Bolya et al. (ICLR 2023): the discarded token blocks at each sparse layer are mean-pooled into a single summary token, the summary token is appended to the kept sequence and propagated through attention, and its attention output is broadcast back onto every dropped position when the residual stream is reconstructed. The modification adds no parameters and the original behavior is byte-identical when the flag is off. Trained from step zero with the new flag active and evaluated on 500 held-out RedPajama documents drawn from beyond the training slice, the modified adapter cuts mean forward loss by **28 percent (2.380 → 1.716)** and approximate perplexity by **48 percent (10.81 → 5.56)** over the original Jenga adapter under hard drop, at essentially zero memory cost and within noise on wall clock. We additionally compose the technique with LongLoRA's shifted attention (the authors' 2D sparsity variant, Figure 19 upper of Jenga) and report a complementary negative result: at the same 2,400-step training budget the composed adapter regresses on quality, bounding the regime in which token-axis and head-axis sparsity freely compound. All trained adapters, evaluation logs, and the artifact's input data are mirrored on a public HuggingFace dataset (`outofaditya/jenga-labs-artifacts`), so every quantitative claim in this report is reproducible from a single bootstrap command.

## 1. Introduction

Fine-tuning a large language model to long context is bounded by activation memory rather than weight memory. At 8 K tokens and above, the per-layer attention and MLP activations for a 7 B-parameter model dwarf both the LoRA optimizer state and the bf16 weights, putting a hard ceiling on what a single GPU can sustain. Existing techniques attack one axis of this ceiling at a time: LoRA (Hu et al., 2022) eliminates the optimizer-state explosion of full fine-tuning; FlashAttention (Dao, 2023) reduces the attention score matrix's wall-clock cost without reducing activation count; LongLoRA (Chen et al., 2024) restructures the attention pattern so adjacent tokens see each other through a shifted block structure. The activation count itself — O(L · N) for L layers at N tokens — was the last untouched axis.

Jenga (Wang et al., USENIX ATC 2025) attacks that axis with **Contextual Token Sparsity**: a small per-block attention predictor scores token-block importance from each layer's input hidden states and the lowest-scoring blocks are removed from both attention and MLP before the heavy matmuls. The paper reports 1.3–2× peak memory reduction and 1.1–1.2× step-time speedup with perplexity within a few percent of dense LoRA, and is, to our knowledge, the first system that prunes tokens *per layer, per batch* from a learned predictor and removes the dropped tokens from both attention and MLP computations without retraining the base model.

This report makes two contributions.

*Reproduction.* §4 is an independent, end-to-end reproduction of every headline claim of the paper that does not require multi-node GPU resources: the memory and time comparisons (Figures 12–13 of the original), the memory and time decomposition (Figure 14), the algorithm ablation across attention and MLP (Figure 15), the predictor convergence curve (Figure 16), the multi-GPU scalability sweep (Figure 17), and the Table 7 perplexity sweep on `proof_pile` and `pg19`. Our numbers match the authors' shipped logs to within approximately one percent on every measured axis.

*Token Merging extension.* §5–6 introduce a runtime modification that softens Jenga's hard token elimination, inspired by Token Merging for vision transformers (Bolya et al., ICLR 2023). At each sparse layer the dropped token blocks are mean-pooled into a single summary token; the summary token is appended to the kept sequence at the position of the last kept token, propagates through the attention and MLP, and its output is broadcast back onto every dropped position when the residual stream is reconstructed. The modification is guarded by `config.merge_eliminated`; when the flag is off the path is byte-identical to the original hard-drop forward, and no new parameters are introduced. The open question is whether the same soft-elimination idea — known to help in ViTs where merged patches preserve a compressed trace of the discarded content — continues to help in a long-context language model whose dropped tokens are, by construction, the ones the predictor scored as low information. We retrain a LoRA adapter from step zero under the merged forward and evaluate on 500 held-out documents drawn from beyond the training slice.

**Headline findings.**

* The reproduction matches the paper's quantitative claims within approximately one percent on every memory and time axis we measured.
* The retrained Token Merging adapter cuts mean forward loss by **28 percent (2.380 → 1.716)** and approximate perplexity by **48 percent (10.81 → 5.56)** over the original Jenga adapter under hard drop, at essentially zero memory cost and within noise on wall clock.
* Composing Token Merging with LongLoRA's shifted attention (the authors' 2D variant) regresses on quality at the same 2,400-step training budget — an honest negative result that delimits the regime in which the two sparsity axes freely compound.

The remainder of the report is organized as follows. §2 places the work in the related literature. §3 lays out the methodology and the research-integrity rules that govern every claim. §4 reports the reproduction in detail. §5 specifies the Token Merging extension. §6 reports the principal evaluation and the 2D composition. §7 discusses what the numbers do and do not say. §8 lists the limitations and §9 concludes. §10 lists every command needed to reproduce the report.

## 2. Background and Related Work

**Long-context fine-tuning constraints.** Self-attention's intermediate score matrix is quadratic in sequence length. FlashAttention (Dao et al., 2022; Dao, 2023) reduces the *wall-clock* cost of this matrix to a streaming O(N), but the *activation* footprint of the q, k, v, and o projections together with the gated MLP remains O(N) per layer and therefore O(L · N) across the L-layer stack. For Llama 2 7B at 8 K tokens and bf16, the activation footprint during fine-tuning is roughly twice that of the bf16 weights themselves; doubling the context length doubles the activations.

**Parameter-efficient adaptation.** LoRA (Hu et al., 2022) sidesteps the optimizer-state explosion of full fine-tuning by training only small rank decompositions of the attention projections. Base weights stay frozen, eliminating their optimizer state, but the activation memory is unchanged. QLoRA (Dettmers et al., 2023) extends this with NF4 base-weight quantization, again leaving activation memory untouched. The artifact's baseline throughout this report is LoRA with r = 8, alpha = 16, applied to `q_proj`, `k_proj`, `v_proj`, and `o_proj` on the Llama 2 7B backbone.

**Sparse attention and KV compression.** Three orthogonal axes have been pursued in the long-context literature. Sparse attention patterns — Longformer (Beltagy et al., 2020), BigBird (Zaheer et al., 2020), and LongLoRA's shifted blocks (Chen et al., 2024) — restrict which keys each query attends to but retain every token in memory. KV-cache compression methods such as KIVI (Liu et al., 2024) and SmoothQuant-derived quantization (Xiao et al., 2023) reduce the *bytes per kept token* but not the count. Jenga (Wang et al., 2025) attacks a different axis: it prunes *per layer, per batch*, using a learned predictor of attention importance, and removes the dropped tokens from both attention and MLP computations without retraining the base model. The concurrent LeMo (Wang et al., 2025, arXiv:2501.09767), by the same first author, generalises the same per-layer token-sparsity construction to a different system stack; the two papers consolidate "Contextual Token Sparsity" as a named technique.

**Soft elimination via Token Merging.** Bolya et al. (ICLR 2023) showed that vision transformers admit a *training-free* Token Merging operation: tokens with similar key vectors are paired by bipartite matching and averaged, and the merged token replaces the discarded one in the kept sequence. The merged token preserves a compressed representation of the discarded patches, and the ViT loses only 0.2–0.3 percent accuracy on ImageNet at 2× throughput. We adapt the central idea — *a soft elimination that retains a pooled trace of the discarded content* — to Jenga's per-layer block sparsity. We do not use bipartite matching: the predictor already decides which blocks to drop, so a second similarity-based decision would introduce a learned component that the equal-budget constraint of §3 cannot afford to tune. We further depart from ToMe in the merging granularity (one summary token per dropped *block*, not per pair) and in the broadcast-back step that is forced by flash-attention's varlen constraint.

**Jenga in three pieces (the parts our work touches).**

1. *Contextual Token Sparsity.* A tiny per-block MLP predictor — two linear layers with a ReLU operating on pooled hidden states — scores the importance of each pool-size-64 token block from the layer input. Layers from index 15 onward keep only the top `config.sparse = 0.4` fraction of blocks; layers 0–14 keep every block and act as a warm-up bank.
2. *Elastic Pattern Predictor.* The predictor is trained offline against pooled attention scores from a frozen base model and reused, frozen, across downstream LoRA fine-tunes (the "elastic" claim). It contributes a constant 656 MB to peak memory at every sequence length (§4.3).
3. *Segmented Loss.* The final cross-entropy over the vocabulary is computed in chunks with intermediate activations discarded between chunks, eliminating a terminal logits memory spike (§4.5).
...
* **Post-hoc broadcast in the 2D composition.** The 2D path uses the post-hoc broadcast variant of Token Merging because the LongLoRA shift requires the kept token count to be divisible by 8. The semantics of this variant are weaker than the in-attention variant used in §5, and the §6.2 negative result conflates the two effects.

## 9. Conclusion

This work performs an end-to-end, independent reproduction of Jenga (Wang et al., USENIX ATC 2025) on a single rented GPU and then asks one focused research question: *what does the dropped half of the sequence look like, if it is not zero?* Our answer — **Token Merging for Jenga**, a one-flag runtime extension that broadcasts a single mean-pooled summary token onto every dropped position at each sparse layer — turns Jenga's hard elimination into a soft elimination and, when the LoRA adapter is retrained from step zero with the new forward active, **cuts mean forward loss by 28 percent and approximate perplexity by 48 percent on 500 held-out RedPajama documents**, at essentially zero memory cost. The clean separation of the per-document loss distributions (Figure 19, no curve crossover) confirms the gain is a population effect, not the variance of a small sample.

We additionally test whether the technique compounds with LongLoRA's shifted attention — the authors' 2D variant — and report a complementary negative result: at the same 2,400-step training budget the composed adapter regresses on the quality axis. The two sparsity mechanisms do not freely compose; the soft elimination idea pays off on the *token axis* where the predictor's signal lives, but not on the *head axis* whose causal-structure cost the predictor was never trained to recover.

The natural follow-up work falls along four axes.

1. *Multi-seed evaluation.* Retrain the Token Merging adapter under three or more seeds at the same step budget and report mean and standard deviation of the 500-document forward loss to characterise the variance of the gain. At n = 500 documents the population effect is unambiguous (Figure 19); the seed sensitivity is the next quantity to estimate.
2. *Windowed perplexity on the Jenga forward.* Build a Jenga-aware perplexity evaluator so the §5 claim can be reported in the paper's own metric (windowed PPL on `proof_pile` and `pg19`) rather than via forward loss as a proxy.
3. *Long-range downstream accuracy.* Evaluate the Token Merging adapter on the LongBench task suite shipped with the artifact (out of scope here), to test whether the forward-loss gain transfers to retrieval and long-range QA.
4. *Compounding investigation.* Re-examine the 2D composition with the *in-attention* variant of Token Merging (rather than the post-hoc broadcast forced by the LongLoRA divisibility constraint), to disentangle the head-split contribution from the merging-variant contribution.

The broader point is that the dropped half of a sparse-attention sequence is not free space. Cheap, parameter-free soft-elimination mechanisms — Token Merging being one of them — can recover a meaningful fraction of the perplexity that hard elimination throws away, *provided* the LoRA weights are jointly adapted to read the merged signal. The trick generalises along the token axis where the predictor's decision lives; whether it transfers to other sparsity axes is the next open question.
...
1. Tuowei Wang, Xingyu Chen, Kun Li, Ting Cao, Ju Ren, and Yaoxue Zhang. *JENGA: Enhancing LLM Long-Context Fine-tuning with Contextual Token Sparsity.* USENIX Annual Technical Conference (ATC) 2025. [usenix.org/conference/atc25/presentation/wang-tuowei](https://www.usenix.org/conference/atc25/presentation/wang-tuowei)
2. Tuowei Wang, Xingyu Chen, Kun Li, Ting Cao, Ju Ren, and Yaoxue Zhang. *LeMo: Enabling LEss Token Involvement for MOre Context Fine-tuning.* arXiv:2501.09767, January 2025. The companion paper to Jenga from the same group; introduces "Contextual Token Sparsity" in the system context.
...
4. Tim Dettmers, Artidoro Pagnoni, Ari Holtzman, and Luke Zettlemoyer. *QLoRA: Efficient Finetuning of Quantized LLMs.* NeurIPS 2023.
5. Daniel Bolya, Cheng-Yang Fu, Xiaoliang Dai, Peizhao Zhang, Christoph Feichtenhofer, and Judy Hoffman. *Token Merging: Your ViT but Faster.* ICLR 2023 (Oral). arXiv:2210.09461.
6. Tri Dao, Daniel Y. Fu, Stefano Ermon, Atri Rudra, and Christopher Ré. *FlashAttention: Fast and Memory-Efficient Exact Attention with IO-Awareness.* NeurIPS 2022.
7. Tri Dao. *FlashAttention-2: Faster Attention with Better Parallelism and Work Partitioning.* arXiv:2307.08691, 2023.
8. Hugo Touvron et al. *Llama 2: Open Foundation and Fine-Tuned Chat Models.* arXiv:2307.09288, 2023.
9. Susan Zhang et al. *OPT: Open Pre-trained Transformer Language Models.* arXiv:2205.01068, 2022.
10. Yukang Chen, Shengju Qian, Haotian Tang, Xin Lai, Zhijian Liu, Song Han, and Jiaya Jia. *LongLoRA: Efficient Fine-Tuning of Long-Context Large Language Models.* ICLR 2024.
11. Iz Beltagy, Matthew E. Peters, and Arman Cohan. *Longformer: The Long-Document Transformer.* arXiv:2004.05150, 2020.
12. Manzil Zaheer et al. *Big Bird: Transformers for Longer Sequences.* NeurIPS 2020.
13. Zirui Liu, Jiayi Yuan, Hongye Jin, Shaochen Zhong, Zhaozhuo Xu, Vladimir Braverman, Beidi Chen, and Xia Hu. *KIVI: A Tuning-Free Asymmetric 2bit Quantization for KV Cache.* ICML 2024.
14. Guangxuan Xiao, Ji Lin, Mickael Seznec, Hao Wu, Julien Demouth, and Song Han. *SmoothQuant: Accurate and Efficient Post-Training Quantization for Large Language Models.* ICML 2023.
